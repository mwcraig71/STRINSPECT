...
Underclearance (`underclearanceData.structureNumber`), and Channel (`channelData.structureNumber`)
are mirrors of it, not independent fields.
...
- Any new module that shows the structure number must edit it through `setStructureNumber`, never write
  its own copy directly. `setStructureNumber` updates global + CIF + UC + Channel and persists all of them.
